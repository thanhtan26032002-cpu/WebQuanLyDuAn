...
const { projectStatusMap, findMember, formatDate, projectSettingsModalOpen, editingProjectId } = useProjectWorkspace()
...
<template>
  ...
  >
    <!-- Gradient Header -->
    <header :class="['px-5 py-5 bg-gradient-to-br relative overflow-hidden', getGradient(project.color)]">
      <!-- Decorative -->
      <div class="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -translate-y-8 translate-x-8 blur-xl"></div>
      
      <div class="relative flex justify-between items-start">
        <div class="flex-1 min-w-0 pr-3">
          <h3 class="font-bold text-white text-base leading-tight mb-2 line-clamp-2">{{ project.name }}</h3>
    ...
    <div class="p-5 flex flex-col flex-1">
      ...
      <div class="mb-4">
        <div class="flex justify-between items-center mb-1.5">
          ...
          <span class="text-sm font-bold text-slate-900">{{ project.progress }}%</span>
        </div>
        <div class="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
          <div 
            :class="['h-full rounded-full transition-all duration-700', getProgressBg(project.color)]"
      ...
      <footer class="flex items-center justify-between">
        ...
        <div class="flex -space-x-1.5">
          <div 
            ...
            :key="memberId" 
            :class="['w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-bold text-white ring-2 ring-white shrink-0', `bg-${findMember(memberId).color}-500`]"
