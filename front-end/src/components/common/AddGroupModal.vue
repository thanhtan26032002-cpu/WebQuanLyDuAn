<script setup>
import { ref } from 'vue'
import { X, Users, Hash, AlignLeft } from '@lucide/vue'
import { useProjectWorkspace } from '../../composables/useProjectWorkspace'

const { addGroupModalOpen, addGroup } = useProjectWorkspace()
...
<template>
  <div v-if="addGroupModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
      <header class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <h2 class="text-xl font-bold text-slate-900 flex items-center gap-2">
          <Users class="w-5 h-5 text-violet-600" /> Tạo nhóm mới
        </h2>
            ...
            >
              ...
              <div class="flex items-center justify-between relative z-10">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <h3 class="text-base font-bold text-slate-900">{{ formData.name || 'Tên nhóm mới...' }}</h3>
